 ...
 *
 * <p>配置项（均支持 Nacos 远程覆盖）：
 *
 * <ul>
 *   <li>{@code nacos.discovery.service-name} — 注册的服务名（默认 {service-name}）
 *   <li>{@code nacos.discovery.service-group} — 服务分组（默认 DEFAULT_GROUP）
 *   <li>{@code nacos.discovery.cluster} — 集群名（默认 DEFAULT）
 *   <li>{@code nacos.discovery.ip} — 注册 IP（可选，为空则自动探测本机非回环地址）
 *   <li>{@code quarkus.http.port} — 注册端口（默认 8080）
 * </ul>
 *
